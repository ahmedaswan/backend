...
class AsyncDatabase:
    def __init__(self):
        self.postgres_uri:pydantic.PostgresDsn = pydantic.PostgresDsn(
            url = f"{setting.DB_POSTGRES_SCHEMA}://{setting.DB_POSTGRES_USER}:{setting.DB_POSTGRES_PASSWORD}@{setting.DB_POSTGRES_HOST}:{setting.DB_POSTGRES_PORT}/{setting.DB_POSTGRES_NAME}"
        )
